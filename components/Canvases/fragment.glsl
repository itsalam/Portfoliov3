...
vec3 color1 = vec3(0.357, 0.357, 0.81);
vec3 color2 = vec3(0.706, 0.706, 0.706);
vec3 color3 = vec3(0.319, 0.69, 0.62);
vec3 color4 = vec3(1.0 ,0.63, 0.38);
...
float pattern_pt2(vec2 q, out vec2 o, out vec2 n){
    ...
    q += 0.05*sin(vec2(0.05)*u_time);
    q *= 0.7 + 0.2*cos(0.005*u_time);
    o = 0.5 + 0.5*fbm_2( q, vec2(1., 6.2), -1., 2., 4 );
    o += .02*sin(vec2(.13, .11)*u_time);
    n = fbm_2( 5.0*o, vec2(9.2, 5.7), 6);
    n *= .85 + 0.10*cos(u_time * 0.2);
    vec2 p = q + n*4.;
