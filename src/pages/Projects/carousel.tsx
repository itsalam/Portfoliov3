...
export default function Carousel(
  props: { visible: boolean } & HTMLProps<HTMLDivElement>
) {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    if (mousePosY && carouselRef.current) {
      ...
      const nextPercentage = Math.max(
        Math.min(prevPercentage + mouseDelta / totalDist, 1),
        0
      );
    ...
    setTimeout(() => {
      if (!mousePosY && carouselRef.current) {
        ...
        (carouselRef.current as HTMLDivElement).animate(
          {
            top: [
              `${-totalDist * prevPercentage + topOffset}px`,
              `${-totalDist + topOffset}px`
            ]
          },
  ...
  const stopAutoScroll = () => {
    if (!mousePosY && carouselRef.current) {
      ...
      const totalDist =
        ...
        topOffset * 2;
      if (currTop) {
        const currProgress = Math.max(
          Math.min((-parseFloat(currTop) + topOffset) / totalDist, 1),
          0
        );
  ...
  return (
    <div
      className={`carousel fixed top-0 h-screen w-80 -translate-x-1/2 ${
        visProp || visible ? '' : 'hidden'
      }`}
