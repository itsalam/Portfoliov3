    ...
    super('DistortEffect', fragmentShader, {
      uniforms: new Map<string, Uniform>([
        ...Object.entries(configs)
          .map<[string, Uniform]>(([name, val]) => [name, new Uniform(val)])
          .concat([
            ['u_time', new Uniform(time)],
...
export default function Background(props: {}) {
  ...
  const { speed, offSet, scale, horizontalOffset, verticalOffset, distort } =
    useControls({
      offSet: 1.75,
      speed: 1,
      scale: 7,
  ...
  const Distort = forwardRef(({}, ref) => {
    ...
    const effect = useMemo(
      () =>
        new DistortEffect({ time: time.current, resolution, ...shaderConfigs }),
      []
    );
  ...
  return (
    <>
      <ambientLight />
      <Html center className="w-screen h-screen bg-base opacity-75" />
