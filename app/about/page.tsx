...
const Page: FC = () => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "px-g-x-1 mb-g-y-4/8 pt-g-y-2-4/8 flex-1 flex font-display text-grid relative w-screen"
      )}
    >
      <motion.div
        layout="position"
        className={cn("flex flex-col w-full relative", {
          "justify-between gap-x-g-y-4/8 px-g-x-1": selectedProject,
        })}
      >
        <LayoutGroup>
          <Track
            className={cn("gap-g-x-1/2", {
              ...
              >
                <div className="overflow-hidden rounded-2xl w-g-x-5 h-full">
                  ...
                  {selectedProject?.name === project.name && (
                    <TextBody className="left-g-x-1-1/2 top-g-y-4/8" />
                  )}
                </div>
                <BackButton
                  onClick={() => changeSelectedProject()}
                  className={cn(
                    "top-0 ml-g-x-1/4 translate-y-1/2 absolute left-0 opacity-0",
          ...
          {!selectedProject && <TextBody className="bottom-g-y-4/8" />}
        </LayoutGroup>
      </motion.div>
    </motion.div>
  );
